...
class LIFO_Policy:
    ...
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        # sorted temporary list by lexographical order
        tmpList = list()
        tmpList.extend(retrievedURLs)
...
# ininiteFIFO fetch policy.
class INF_FIFO_Policy:
    def __init__(self, c):
        self.queue = copy.deepcopy(c.seedURLs)
...
class Container:
    def __init__(self):
        ...
        self.generatePolicy = INF_LIFO_Policy(self)
...
def getNormalisedURLs(retrievedURLs):
    y=[x.lower() for x in retrievedURLs]
    #print("SDADSDSASS", y)
    return y
...
def removeDuplicates(c, retrievedURLs):
    dict = copy.deepcopy(retrievedURLs)
    for i in dict:
        if i in c.URLs:
            retrievedURLs.remove(i)
...
def storeURLs(c):
    relBeginIndex = len(c.rootPage)
    totalPath = "C:/Users/mikol/Desktop/Tomczyk/lab1/" + c.example + "/urls/urls.txt"
...
def storeOutgoingURLs(c):
    relBeginIndex = len(c.rootPage)
    totalPath = "C:/Users/mikol/Desktop/Tomczyk/lab1/" + c.example + "/outgoing_urls/outgoing_urls.txt"
