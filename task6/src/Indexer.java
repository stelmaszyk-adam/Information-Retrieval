...
public class Indexer
{
    ...
    private void indexDocuments() throws IOException {
        ...
        IndexWriter indexWriter = new IndexWriter(
                FSDirectory.open(Paths.get(Constants.index_dir)) ,  indexWriterConfig);
    ...
    private ArrayList<Document> getHTMLDocuments()
    {
        ...
        if (files != null)
        {
            ...
            for (int id = 0; id < files.length; id++)
            {
                ...
                htmls.add(getHTMLDocument("src/pages/" + files[id].getName(), id));
            }
    ...
    private Document getHTMLDocument(String path, int id)
    {
        ...
        */
        ...
        StoredField storedField = new StoredField(Constants.id, file.getName());
        // ----------------------------------
        ...
        StringReader stringReader = new StringReader(getTextFromHTMLFile(file));
        TextField textField = new TextField(Constants.content,stringReader);
        ...
        // ----------------------------------
        StoredField storedField2 = new StoredField(Constants.filename, file.getName());
        ...
        // ----------------------------------
        IntPoint intPoint = new IntPoint(Constants.filesize_int, (int) file.length());

        // ----------------------------------
        // //TODO IntPoint is not stored but we want to a file size
        // ... so add another field (StoredField) that stores file size
        // ----------------------------------
        StoredField storedField3 = new StoredField(Constants.filesize, file.length());
